...
if [ "$TEST_AGAINST_PIWIK_BRANCH" == "" ]; then
    if [ "$TEST_AGAINST_CORE" == "latest_stable" ]; then # test against the latest stable release of Matomo core (including betas & release candidates)
        ...
        export TEST_AGAINST_PIWIK_BRANCH=$(php "$SCRIPT_DIR/../php/get_required_matomo_version.php" $WORKSPACE $PLUGIN_NAME)
    ...
    export TEST_AGAINST_PIWIK_BRANCH=$(php "$SCRIPT_DIR/../php/get_required_matomo_version.php" $WORKSPACE $PLUGIN_NAME "max")
