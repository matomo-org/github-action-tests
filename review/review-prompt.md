...
- Do not report findings against the Matomo core context itself unless the issue is caused by the
  PR diff. Inline comments must use the actual changed file paths from the PR diff, not the
  `{{PLUGIN_IN_CORE_PATH}}` mapping path.
- Base SHA: `{{BASE_SHA}}`
- Head SHA: `{{HEAD_SHA}}`
- Base ref: `{{BASE_REF}}`
- Head ref: `{{HEAD_REF}}`
